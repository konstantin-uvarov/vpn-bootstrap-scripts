#!/usr/bin/env bash
#
# AmneziaWG VPN Server - Bootstrap Script
# Downloads and sets up the AmneziaWG VPN server on a fresh Linux VM
#
# Usage:
#   curl -sSL https://raw.githubusercontent.com/konstantin-uvarov/vpn-bootstrap-scripts/main/bootstrap-amnezia-wg.sh | bash
#   # Or download first:
#   curl -sSL https://raw.githubusercontent.com/konstantin-uvarov/vpn-bootstrap-scripts/main/bootstrap-amnezia-wg.sh -o bootstrap.sh && bash bootstrap.sh
#
...
DEFAULT_REPO="https://github.com/konstantin-uvarov/docker-amnezia-wg.git"
...
check_environment() {
    ...
    if is_wsl2; then
        ...
        if [[ ! "$confirm" =~ ^[Yy]$ ]]; then
            log_info "Aborting. Use 'make start' on a cloud VM instead."
            exit 0
        fi
    fi
...
main() {
    echo "" >&2
    echo "=============================================" >&2
    echo "  AmneziaWG VPN Server - Bootstrap Script" >&2
    echo "=============================================" >&2
    echo "" >&2
    ...
    echo "" >&2
    echo "Next steps:" >&2
    echo "  - Scan QR code with AmneziaVPN app (not WireGuard!)" >&2
    echo "  - Add more peers: make add-peer NAME=<name>" >&2
    echo "  - View status: make status" >&2
    echo "  - View logs: make logs" >&2
    echo "" >&2
    echo "Client downloads:" >&2
    echo "  - Android: https://play.google.com/store/apps/details?id=org.amnezia.vpn" >&2
    echo "  - iOS: https://apps.apple.com/app/amneziavpn/id1600529900" >&2
    echo "" >&2
}
